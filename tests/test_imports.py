...
class TestImports:
    ...
    def test_import_models(self):
        from pyiikocloudapi.models import CustomErrorModel, BaseOrganizationsModel
...
class TestMRO:
    ...
    def test_mro_order_matches(self):
        ...
        from pyiikocloudapi import IikoTransport, AsyncIikoTransport
    ...
    def test_async_api_methods_are_coroutines(self):
        ...
        for name, method in inspect.getmembers(AsyncIikoTransport, predicate=inspect.isfunction):
            ...
            assert inspect.iscoroutinefunction(method), (
                f"Method {name} should be a coroutine but is not"
            )
        ...
        for path in self._get_python_sources():
            ...
            for i, line in enumerate(content.splitlines(), 1):
                ...
                assert 'import requests' not in stripped, (
                    f"{path.name}:{i} still contains 'import requests'"
                )
        ...
        for path in self._get_python_sources():
            ...
            for i, line in enumerate(content.splitlines(), 1):
                ...
                assert 'requests.Session' not in stripped, (
                    f"{path.name}:{i} still contains 'requests.Session'"
                )
    ...
    def test_no_requests_exceptions(self):
        for path in self._get_python_sources():
            ...
            for i, line in enumerate(content.splitlines(), 1):
                ...
                if stripped.startswith('#') or stripped.startswith('"') or stripped.startswith("'"):
                    continue
                assert 'requests.exceptions' not in stripped, (
                    f"{path.name}:{i} still contains 'requests.exceptions'"
                )
